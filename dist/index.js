...
// MCP server setup
// ──────────────────────────────────────
const server = new Server({ name: "drop-beacon-mcp", version: "0.2.0" }, { capabilities: { tools: {} } });
const API_KEY_PARAM = {
    ...
    try {
        ...
        switch (name) {
            case "search_products":
                ...
                break;
            case "get_latest_drops":
                result = await getLatestDrops(GetLatestDropsSchema.parse(args ?? {}));
                break;
            case "get_brand_info":
                result = await getBrandInfo(GetBrandInfoSchema.parse(args ?? {}));
                break;
            case "get_price_comparison":
                result = await getPriceComparison(GetPriceComparisonSchema.parse(args ?? {}));
                break;
            case "get_market_trends":
                result = await getMarketTrends(GetMarketTrendsSchema.parse(args ?? {}));
                break;
            case "check_availability":
                result = await checkAvailability(CheckAvailabilitySchema.parse(args ?? {}));
                break;
